...
import { resolvePendingTaskConflict } from "../sync/TaskConflictPolicy";
import { UnifiedSyncEngine } from "../sync/UnifiedSyncEngine";
import { SyncOutbox } from "../sync/SyncOutbox";
import { RemoteCommandExecutor } from "../sync/RemoteCommandExecutor";
...
export class TaskSyncService {
    ...
    private projectionQueue: PersistentProjectionQueue;
    private remoteSnapshotLoader: TaskRemoteSnapshotLoader;

    constructor(plugin: DidaSyncPlugin) {
        this.plugin = plugin;
        this.outbox = new SyncOutbox(plugin);
        this.remoteCommandExecutor = new RemoteCommandExecutor(this.outbox);
        this.projectionQueue = new PersistentProjectionQueue(plugin);
        this.remoteSnapshotLoader = new TaskRemoteSnapshotLoader(plugin);
    ...
    getSyncState(): SyncRunState {
        return this.unifiedSyncEngine?.getState() || this.syncRunCoordinator.getState();
    }

    attachUnifiedSyncEngine(engine: UnifiedSyncEngine) {
        this.unifiedSyncEngine = engine;
    }
    ...
    async queueOperation(task: DidaTask, type: PendingSyncOperationType, payload?: PendingSyncOperation["payload"]) {
        ...
        await this.outbox.enqueue(task, type, payload);
    }
    ...
    async markOperationFailed(task: DidaTask, error: any) {
        const operation = this.outbox.findForTask(task);
        if (operation) await this.outbox.markFailed(operation, error);
    }
    ...
    async flushPendingOperations(): Promise<{ uploaded: number; failed: string[]; failures: SyncFailureDetail[] }> {
        ...
        for (const operation of this.sortPendingOperationsForFlush([...this.getPendingOperations()])) {
            ...
            try {
                await this.remoteCommandExecutor.execute(operation, async () => {
                    if (task && this.plugin.isNoteSyncTaskLike && this.plugin.isNoteSyncTaskLike(task)) return;
                    if (operation.type === "delete") {
                        if (operation.didaId) await this.deleteTaskInDidaList(operation.didaId, operation.projectId || "inbox", false);
    ...
    private async removeOperation(operation: PendingSyncOperation) {
        await this.outbox.remove(operation);
    }
    ...
    async runBidirectionalSync(): Promise<SyncResult> {
        ...
        if (this.unifiedSyncEngine) {
            return this.unifiedSyncEngine.run({ source: "manual", scope: "tasks" });
        }
    ...
    async runTaskScope(context: SyncRunContext): Promise<SyncResult> {
        return this.runSingleBidirectionalSync(context);
    }
    ...
    async syncFromDidaList(lockHeld: boolean = false): Promise<SyncResult> {
        ...
        try {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (Array.isArray(list)) {
                        ...
                        const taskProjects = list.filter(project => !this.plugin.isNoteProjectLike(project));
                        const projectSnapshot = await this.remoteSnapshotLoader.fetchProjects(
                            taskProjects,
                            projectMap,
                            projectId => this.normalizeRemoteProjectId(projectId)
                        );
                        tasks.push(...projectSnapshot.tasks);
                        projectSnapshot.successfulProjectIds.forEach(projectId => successfulProjects.add(projectId));
                        result.failedScopes.push(...projectSnapshot.failures);
                    }
                }
            } catch (e) { }
            ...
            try {
                ...
                const inboxSnapshot = await this.remoteSnapshotLoader.fetchInbox(
                    remoteInboxProjectId,
                    projectMap.get("inbox"),
                    projectId => this.normalizeRemoteProjectId(projectId),
                    projectId => this.isInboxProjectId(projectId)
                );
                inboxSucceeded = inboxSnapshot.succeeded;
                const ids = new Set(tasks.map(task => task.id));
                tasks.push(...inboxSnapshot.tasks.filter(task => !ids.has(task.id)));
                if (inboxSnapshot.failure) result.failedScopes.push(inboxSnapshot.failure);
            } catch (e) { }
                ...
                for (const remote of tasks) {
                    ...
                    if (idx === -1) {
                        ...
                        if (pendingOperation && remoteKind !== "NOTE") {
                            ...
                            if (decision === "unresolvable") {
                                ...
                                result.failedDetails = [...(result.failedDetails || []), {
                                    ...
                                    reason,
                                    attempts: pendingOperation.attempts
                                }];
                                continue;
                            }
                        ...
                        if (remote.title && remote.title !== local.title) {
                            ...
                            changed = true;
                            if (local.didaId) {
                                this.projectionQueue.enqueueTask(local.didaId, local.linkPath, { oldTitle, title: remote.title });
                            }
                        }
                        ...
                        if (mergedSchedule.dueDate !== local.dueDate) {
                            ...
                            changed = true;
                            if (local.didaId) {
                                this.projectionQueue.enqueueTask(local.didaId, local.linkPath, { oldDue, dueDate: mergedSchedule.dueDate });
                            }
                        }
                        ...
                        if (remote.priority !== undefined && remote.priority !== local.priority) {
                            ...
                            changed = true;
                            if (local.didaId) {
                                this.projectionQueue.enqueueTask(local.didaId, local.linkPath, { refreshSchedule: true });
                            }
                        }
            ...
            await this.flushPendingProjections();
    ...
    dispose() {
        ...
        this.unifiedSyncEngine?.dispose();
    ...
    private async flushPendingProjections() {
        for (const operation of [...this.projectionQueue.list()]) {
            const task = this.plugin.settings.tasks.find(item => item.didaId === operation.remoteId);
            if (!task) {
                await this.projectionQueue.remove(operation);
                continue;
            }
            try {
                this.plugin.isReverseUpdating = true;
                const payload = operation.payload || {};
                for (const leaf of this.plugin.app.workspace.getLeavesOfType(TASK_VIEW_TYPE)) {
                    if (!(leaf.view instanceof TaskView)) continue;
                    if ("title" in payload && (leaf.view as any).updateNativeTaskTitle) {
                        await Promise.resolve((leaf.view as any).updateNativeTaskTitle(task, payload.oldTitle, payload.title));
                    }
                    if (("dueDate" in payload || payload.refreshSchedule) && (leaf.view as any).updateNativeTaskDueDate) {
                        await Promise.resolve((leaf.view as any).updateNativeTaskDueDate(task, payload.oldDue ?? task.dueDate, payload.dueDate ?? task.dueDate));
                    }
                }
                await this.projectionQueue.remove(operation);
            } catch (error) {
                await this.projectionQueue.markFailed(operation, error);
            } finally {
